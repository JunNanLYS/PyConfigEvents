...
class DataModel(BaseModel):
    ...
    def subscribe(self, field: str, callback: Callable) -> None:
        ...
        if field not in self.__class__.model_fields:
            raise ValueError(f"Field {field} does not exist in {self.__class__.__name__}")
    ...
    def __setattr__(self, name: str, value: Any, /) -> None:
        ...
        if name in self.__class__.model_fields:
            ...
            if type(value) is not type(old_value):
                raise TypeError(f"Field <{name}> type {type(value)} is not compatible with {type(old_value)}")
            ...
            raise AttributeError(f"Field <{name}> does not exist in {self.__class__.model_fields}")
